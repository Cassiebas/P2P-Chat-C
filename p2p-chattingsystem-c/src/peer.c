...
void name_request(char *name) {
	...
	memcpy(my_name, name, sizeof(my_name));
	// format packet
	packet pkt;
	pkt.header.type = 'n';
	pkt.header.error = '\0';
...
void send_message(char *msg) {
	...
	pkt.header.error = '\0';
	pkt.header.room = room_num;
	memcpy(pkt.header.name, my_name, sizeof(pkt.header.name));
...
void receive_message(struct sockaddr_in *sender_addr, packet *pkt) {
	...
	if (pkt->header.room == room_num) {
		pthread_mutex_lock(&stdout_lock);
		printf("%s @ %s:%d - %s\n", pkt->header.name, sender_ip, sender_port, pkt->payload);
		pthread_mutex_unlock(&stdout_lock);
	}
}
